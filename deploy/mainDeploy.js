...
async function main() {
  ...
  const {doc, rbtc, sat } = config.markets;
  ...
  let unitroller = await ethers.getContractAt('Unitroller', unitrollerDeployed.address, deployer);
  ...
  unitroller = await ethers.getContractAt('ComptrollerG6', unitrollerDeployed.address, deployer);
  await unitroller._setPriceOracle(priceOracleProxyDeploy.address).then((tx) => tx.wait());
  await unitroller._setCloseFactor(config.closeFactorMantissa).then((tx) => tx.wait());
  await unitroller._setLiquidationIncentive(config.liquidationIncentiveMantissa).then((tx) => tx.wait());
  // console.log('\n~~~~~~~~~~~~~~~~~ UNITROLLER & COMPTROLLER ~~~~~~~~~~~~~~~~');
  ...
  const cDOCdeployed = await cErc20Immutable.deploy(docToken.address, unitrollerDeployed.address, docInterestRateModel.address, config.initialExchangeRateMantissa, 'Tropykus kDOC', 'kDOC', 18, deployer.address);
  await cDOCdeployed.deployTransaction.wait();
  console.log(`cDOC = '${cDOCdeployed.address}';`);
  const cRBTCdeployed = await cRBTCContract.deploy(unitrollerDeployed.address, rbtcInterestRateModel.address, config.initialExchangeRateMantissa, 'Tropykus kRBTC', 'kRBTC', 18, deployer.address);
  await cRBTCdeployed.deployTransaction.wait();
  console.log(`cRBTC = '${cRBTCdeployed.address}';`);
  const cSATdeployed = await cRBTCContract.deploy(unitrollerDeployed.address, satInterestRateModel.address, config.initialExchangeRateMantissa, 'Tropykus kSAT', 'kSAT', 18, deployer.address);
  await cSATdeployed.deployTransaction.wait();
  console.log(`cSAT = '${cSATdeployed.address}';`);
  const cRBTCCompanionDeployed = await cRBTCCompanionContract.deploy(unitrollerDeployed.address, cSATdeployed.address, priceOracleProxyDeploy.address);
  ...
  // console.log('~~~~~~~~~~~~~~~~~ /COMPTROLLER MARKET INTEGRATION ~~~~~~~~~~~~~~~~\n');
  ...
  await unitroller._supportMarket(cDOCdeployed.address).then((tx) => tx.wait());
  await unitroller._supportMarket(cRBTCdeployed.address).then((tx) => tx.wait());
  await unitroller._supportMarket(cSATdeployed.address).then((tx) => tx.wait());

  await unitroller._setCollateralFactor(cDOCdeployed.address, doc.collateralFactor).then((tx) => tx.wait());
  await unitroller._setCollateralFactor(cRBTCdeployed.address, rbtc.collateralFactor).then((tx) => tx.wait());
  await unitroller._setCollateralFactor(cSATdeployed.address, sat.collateralFactor).then((tx) => tx.wait());

  await unitroller._setCompRate(config.compSpeed);
